...
class ProductInShop(viewsets.ModelViewSet):
    queryset = Product.objects.all()
    ...
    if request.method == "POST":
        ...
        if value.is_valid():
            ...
            product = Product.objects.create(
                name=new_name, description=new_description, category_id=new_category
            )
...
class ShopForm(forms.Form):
    ...
    shop_owner = forms.CharField(
        required=True,
        widget=forms.Select(
            choices=[(vendor.id, vendor.name) for vendor in Vendor.objects.all()]
        ),
    )
...
def addShop(request):
    if request.method == "POST":
        ...
        if value.is_valid():
            ...
            Shop.objects.create(name=new_name, owner_id=new_owner)
